...
>(
    ...
    let custom_builtins = options.custom_builtins.clone();
    ...
    run_spec_checker(&mut env, units, expansion_ast, &custom_builtins);
    Ok(env)
}
...
fn run_spec_checker(
    ...
    mut eprog: E::Program,
    custom_builtins: &[String],
) {
    let mut builder = ModelBuilder::new(env, custom_builtins);
